function summarizePerson(
  ...
  const fullName = middleName
    ...
    : `${firstName} ${lastName}`;
  const hobbiesAsStr = hobbies && hobbies.length ? hobbies.join(", ") : "-";
  const workInfoAsStr = workInfo ? `${workInfo[0]} -> ${workInfo[1]}` : "-";

  return [id, fullName, age, hobbiesAsStr, workInfoAsStr];
}
